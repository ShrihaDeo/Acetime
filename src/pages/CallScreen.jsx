...
function CallScreen({ socket, room, nickname, onLeave }) {
  ...
  useEffect(() => {
    ...
    socket.on("receive-move", (data) => {
      setSyncStatus(
        `${opponentNickname} played ${data.cardValue}${data.cardSuit}`,
      );
    });
    ...
    socket.on("camera-status", ({ isCameraOff: off }) =>
      setIsOpponentCameraOff(off),
    );
    ...
    socket.on("last-card-called", ({ playerID }) => {
      ...
      const name =
        playerID === socket.id ? "You" : nicknames[playerID] || "Opponent";
      ...
      {showRules && (
        ...
        >
          ...
          >
            ...
            >
              Match the top card by suit or rank. First to empty their hand
              wins. If you can't play, draw one card and your turn ends.
            </p>
