...
//NOTE: Moved these two functions outside of the registerCardhandlers because they were causing errors when trying to run the code 
export function startLastCard(io, room, roomStates) {
  ...
  if (roomStates[room]) {
    console.log('game already started for room:', room);
    return;
  }
...
export function registerLastCardHandlers(io, socket, roomStates) {
  //(the whole server, this specific player's connection, shared object that stores game state per room)
