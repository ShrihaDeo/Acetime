...
export function playTurn(state, selectedGame, playerID, cardPlayed) {
  switch (selectedGame) {
    case "LastCard": {
      ...
      if (cardIndex === -1) {
        const { newState } = drawCard(state, playerID, selectedGame); //Previously the newState return was being discarded
        return { newState, error: "Card not in hand!" };
      }
...
export function applyCardEffect(state, playerID, card, selectedGame) {
  switch (selectedGame) {
    case "LastCard": {
      if (card.value === "2") {
        const newState = {...state, drawStack: state.drawStack + 2, log: playerID + " player 2- draw stack is now" + (state.drawStack + 2),} //previously turn wasnt advancing after applying the card effect 
